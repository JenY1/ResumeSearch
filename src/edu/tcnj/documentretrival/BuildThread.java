...
import resume.index.Document;
import resume.index.ResumeReader;
...
public class BuildThread extends Thread
	...
	public void run()
		...
		for (int i = 0; i < files.length; i++)
			...
			ResumeReader reader = new ResumeReader(files[i]);

			// loop through Documents, one-by-one
				if ((c = reader.readDocument()) != null)
					{
		...
		if (invInd != null && stats.currentDocument != null)
			invInd.build(stats);
		if (docInd != null && stats.currentDocument != null)
			docInd.build(stats);
		}
	}
